 ...
 */
export function maxDrawdownFromValues(points: ValuePoint[]): DrawdownResult | null {
  if (points.length < 2) return null
  const sorted = [...points].sort((a, b) => a.timestampMs - b.timestampMs)
 ...
 * Returns null if fewer than MIN_SAMPLES returns.
 */
export function historicalCVaR(returns: number[], confidence: number): number | null {
  if (returns.length < MIN_SAMPLES) return null
  const sorted = [...returns].sort((a, b) => a - b)
 ...
 */
export function parametricVaR(returns: number[], confidence: number): number | null {
  if (returns.length < MIN_SAMPLES) return null
  const variance = sampleVariance(returns)
  if (variance === null || variance <= 0) return null
