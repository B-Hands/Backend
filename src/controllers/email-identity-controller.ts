...
export async function requestEmailVerification(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const existing = await db.emailIdentity.findUnique({ where: { email: normalizedEmail } });
    if (existing && existing.userId !== userId && existing.status === 'VERIFIED') {
      // Return generic success message to prevent account enumeration
      res.json({ success: true, message: 'Verification email sent if address is valid' });
      return;
    }
    ...
    logger.error('[EmailIdentityController] Failed to request email verification', { error: err.message });
    res.status(500).json({ error: 'Failed to process email verification request' });
  }
}
...
export async function verifyEmail(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    logger.error('[EmailIdentityController] Failed to verify email token', { error: err.message });
    res.status(500).json({ error: 'Failed to verify email' });
  }
}

export async function handleMailWebhook(req: Request, res: Response): Promise<void> {
  try {
    const event = mailRegistry.parseWebhook(req.body, req.headers['x-signature'] as string);
    ...
    if (event.type === 'bounce' || event.type === 'complaint') {
      ...
      if (identity) {
        ...
        await publishUserEvent(identity.userId, 'alerts', 'notification.email_suppressed' as any, {
          email: identity.email,
    ...
    logger.error('[EmailIdentityController] Failed to process mail webhook', { error: err.message });
