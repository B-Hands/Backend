...
export async function snoozeAlertRule(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (until) {
      ...
      res.status(400).json({ error: 'Must provide durationMinutes or until date' });
      return;
    }
    ...
    logger.error('[AlertAckController] Failed to snooze alert rule', { error: err.message });
...
export async function acknowledgeAlert(req: Request, res: Response): Promise<void> {
  ...
  try {
    const rule = await db.alertRule.findFirst({ where: { id: targetRuleId, userId: targetUserId } });
    if (!rule) {
      res.status(404).json({ error: 'Alert rule not found' });
      return;
    }
    ...
    await publishUserEvent(targetUserId, 'alerts', 'alert_rule.acknowledged' as any, {
      ruleId: targetRuleId,
      fireId: targetFireId,
      ackId: ack.id,
    ...
    logger.error('[AlertAckController] Failed to acknowledge alert', { error: err.message });
    res.status(500).json({ error: 'Failed to acknowledge alert' });
  }
}

export async function listAlertFires(req: Request, res: Response): Promise<void> {
  const userId = (req as any).user?.id || (req as any).userId;
  const { id } = req.params;
