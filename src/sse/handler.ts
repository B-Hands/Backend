...
import { USER_EVENT_TOPICS, type UserEventTopic, type UserEventEnvelope } from '../events/types';
...
export async function handleSseConnection(req: Request, res: Response): Promise<void> {
  const auth = await authenticateStreamRequest(req.headers as any, req.query as any);
  if (!auth.ok || !auth.viewerUserId || !auth.streamUserId || !auth.allowedTopics) {
    res.status(401).json({ error: auth.error || 'Unauthorized' });
    return;
  }

  // Parse requested topics
  const requestedTopicsStr = typeof req.query.topics === 'string' ? req.query.topics : '';
  ...
  const activeTopics = requestedTopics.filter((t) => auth.allowedTopics!.includes(t));
  ...
  let afterSeq = 0;
  if (typeof lastEventIdHeader === 'string' && !isNaN(parseInt(lastEventIdHeader, 10))) {
    afterSeq = parseInt(lastEventIdHeader, 10);
  } else if (typeof afterSeqParam === 'string' && !isNaN(parseInt(afterSeqParam, 10))) {
  ...
  if (afterSeq > 0) {
    try {
      ...
      logger.warn('[SSE] Failed to replay retained events', { error: err.message });
    }
  }
