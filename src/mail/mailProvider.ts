...
export class MockMailProvider implements MailProvider {
  ...
  async send(message: MailMessage): Promise<MailSendResult> {
    ...
    logger.info(`[MockMailProvider] Sent email to ${message.to}: ${message.subject}`);
...
export class SmtpMailProvider implements MailProvider {
  ...
  parseWebhook(rawPayload: any): MailWebhookEvent | null {
    ...
    return {
      type: rawPayload.event === 'bounce' ? 'bounce' : rawPayload.event === 'complaint' ? 'complaint' : 'delivery',
...
export class SesMailProvider implements MailProvider {
  ...
  parseWebhook(rawPayload: any): MailWebhookEvent | null {
    ...
    const type = notificationType === 'bounce' ? 'bounce' : notificationType === 'complaint' ? 'complaint' : 'delivery';
    ...
    return {
      ...
      recipient,
      reason: rawPayload.bounce?.bounceType || rawPayload.complaint?.complaintFeedbackType,
    };
  }
}
...
export class MailRegistry {
  ...
  async send(message: MailMessage): Promise<MailSendResult> {
    if (this.isHealthy) {
      try {
        ...
        logger.warn(`[MailRegistry] Primary mail provider "${this.primaryProvider.name}" failed, failing over to fallback`, { error: err.message });
        this.isHealthy = false;
        // Attempt recovery after 60s
        setTimeout(() => { this.isHealthy = true; }, 60000);
  ...
  parseWebhook(rawPayload: any, signature?: string): MailWebhookEvent | null {
    return this.primaryProvider.parseWebhook(rawPayload, signature) || this.fallbackProvider.parseWebhook(rawPayload, signature);
  }
}
