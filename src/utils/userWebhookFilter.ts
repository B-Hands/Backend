...
export function validateFilterPredicate(filter: any, depth = 1): boolean {
  ...
  if (depth > MAX_FILTER_DEPTH) {
    throw new Error(`Filter predicate depth exceeds maximum allowed (${MAX_FILTER_DEPTH})`);
  }
  ...
  if (!filter.op || !validOps.includes(filter.op)) {
    throw new Error(`Filter condition "op" must be one of: ${validOps.join(', ')}`);
  }

  if (filter.op === 'in' && !Array.isArray(filter.value)) {
    throw new Error('Filter condition with "in" op must provide an array "value"');
  }
 ...
 */
export function evaluateFilterPredicate(filter: FilterCondition | null | undefined, payload: Record<string, any>): boolean {
