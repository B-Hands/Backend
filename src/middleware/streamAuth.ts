...
const STREAM_TICKET_SECRET = process.env.JWT_SECRET || 'neuro-stream-ticket-secret-key-32b';
 ...
 */
export function verifyStreamTicket(ticketToken: string): StreamTicketPayload | null {
  try {
    ...
    if (usedTicketIds.has(payload.ticketId)) {
      logger.warn('[StreamAuth] Stream ticket replay attempt detected', { ticketId: payload.ticketId });
      return null;
    }
...
export async function authenticateStreamRequest(
  ...
  if (ticket) {
    ...
    if (!ticketPayload) {
      return { ok: false, error: 'Invalid, expired, or already-used stream ticket' };
    }
    return {
      ok: true,
