...
export function validateSsrfUrl(urlString: string): void {
  ...
  if (privateHostPatterns.some((pattern) => pattern.test(hostname))) {
    throw new Error(`Forbidden webhook URL: hostname "${hostname}" resolves to a private or loopback range`);
  }
}
 ...
 */
export function signUserWebhookPayload(payloadString: string, secret: string, timestamp: number = Math.floor(Date.now() / 1000)): string {
...
export async function enqueueUserWebhooks(
  ...
  try {
    ...
    for (const endpoint of endpoints) {
      ...
      // Check topic scope
      if (endpoint.topicScope.length > 0 && !endpoint.topicScope.includes(topic)) {
        continue;
      }

      // Check server-side filter predicate
      if (endpoint.filterJson && !evaluateFilterPredicate(endpoint.filterJson as any, redactedPayload)) {
        continue;
      }
      ...
      try {
        ...
        if (err?.code !== 'P2002') {
          logger.warn('[UserWebhookDispatcher] Failed to create delivery record', {
            endpointId: endpoint.id,
            seq,
            error: err.message,
          });
